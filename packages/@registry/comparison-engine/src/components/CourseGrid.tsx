...
export const CourseGrid = ({ providers: incomingProviders }: CourseGridProps) => {
    ...
    const providers = (incomingProviders && incomingProviders.length > 0) ? incomingProviders : mockProviders;

    const filtered = providers.filter((p: any) =>
        p.name.toLowerCase().includes(search.toLowerCase()) ||
        p.rto.toLowerCase().includes(search.toLowerCase())
    );
                ...
                {filtered.map((provider: any, i: number) => (
                    <div key={i} className="group relative bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-500 overflow-hidden flex flex-col">
                        ...
                        <div className="p-6 pb-0 flex gap-2">
                            {provider.badges && provider.badges.map((badge: string, bi: number) => (
                        ...
                        <div className="p-8 flex-grow">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                {provider.rating && (
                                    <div className="bg-yellow-400 text-white font-black px-3 py-1 rounded-lg text-sm">
                                        {provider.rating}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700">
                                ...
                                <ul className="grid grid-cols-1 gap-3">
                                    {provider.features && provider.features.map((feature: string, fi: number) => (
