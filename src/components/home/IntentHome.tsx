...
export function IntentHome({ qualification, faqs, research }: IntentHomeProps) {
    ...
    // Helper for grid stats
    const getCardStat = (slug: string) => {
        ...
        switch (slug) {
             ...
             case "team-leadership":
                if (research.salary?.median) {
                    return { 
                        label: "Median Salary", 
                        value: `$${research.salary.median.toLocaleString()}`,
                        icon: <TrendingUp className="w-3 h-3 text-emerald-500" />
                    };
                }
                return { label: "High Demand", value: "Growth Role", icon: <TrendingUp className="w-3 h-3 text-emerald-500" /> };
            ...
            case "funding-options":
                if (research.pricing?.min) {
                    return { 
                        label: "From", 
                        value: `$${research.pricing.min.toLocaleString()}`,
                        icon: <DollarSign className="w-3 h-3 text-emerald-500" /> 
                    };
                }
                return { label: "Gov Funding", value: "Subsidies Available", icon: <DollarSign className="w-3 h-3 text-emerald-500" /> };

            case "study-options":
            case "pathway-to-diploma":
                if (research.duration?.min) {
                    return { 
                        label: "Duration", 
                        value: `${research.duration.min}-${research.duration.max} ${research.duration.unit}`,
                        icon: <Clock className="w-3 h-3 text-blue-500" />
                    };
                }
                return { label: "Pacing", value: "Self-Paced Flexible", icon: <Clock className="w-3 h-3 text-blue-500" /> };
            ...
            <section className="relative pt-32 pb-16 md:pt-40 md:pb-32 border-b border-slate-100 overflow-hidden">
                <div className="container px-4 relative">
                    <div className="max-w-5xl mx-auto text-center mb-24">
                        <div className="flex flex-col md:flex-row items-center justify-center gap-4 mb-10 animate-fade-in">
                            <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-slate-50 border border-slate-200 text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] shadow-sm">
                                <ShieldCheck className="w-4 h-4 text-primary" />
                                <span>National Registry Node</span>
                            </div>
                            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-600 text-[10px] font-black uppercase tracking-widest shadow-sm">
                                <span className="relative flex h-2 w-2 mr-1">
                                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                                </span>
                                {market_snapshot.provider_count} Providers Live
                            </div>
                        </div>
                        ...
                        <div className="mt-8">
                             <Link href="/compare" className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-full font-black text-sm uppercase tracking-widest hover:bg-primary transition-all shadow-xl hover:-translate-y-1">
                                Launch Terminal
                    ...
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
                            {[
                                ...
                                const stat = getCardStat(topic.slug);
                                return (
                                    <Link
                                        key={topic.slug}
                                        href={`/tag/${topic.slug}`}
                                        className="group p-8 bg-white rounded-3xl border border-slate-100 hover:border-primary/30 hover:shadow-2xl transition-all text-center flex flex-col items-center h-[240px] justify-between relative overflow-hidden"
                                    >
                                        <div className={cn(
                                        ...
                                        {/* Micro-Data Reveal */}
                                        <div className="absolute inset-x-0 bottom-0 p-4 bg-slate-50/90 backdrop-blur-sm transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex flex-col items-center justify-center border-t border-slate-100">
                                            {stat ? (
                                                <>
